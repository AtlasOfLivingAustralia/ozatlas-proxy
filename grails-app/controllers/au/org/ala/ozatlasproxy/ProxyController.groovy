...
class ProxyController {
    ...
    def image = {
        ...
        try {
            def url = ("https://bie-ws.ala.org.au/ws/species/image/" + params.imageType + "/" + params.guid).toURL()
            url.getBytes()
    ...
    def geocode = {
        def url = ("https://maps.googleapis.com/maps/api/geocode/json?sensor=true&latlng=" + params.latlng).toURL()
        response.setContentType("application/json")
        render url.getText()
    }

    def placeSearch = {
        def url = ("https://maps.googleapis.com/maps/api/geocode/json?sensor=true&address=" + params.address.encodeAsURL()).toURL()
        response.setContentType("application/json")
        render url.getText()
    }

    def exploreGroups = {
        def url = ("https://biocache-ws.ala.org.au/ws/explore/groups.json?fq=geospatial_kosher%3Atrue&facets=species_group&lat=" + params.lat + "&lon=" + params.lon + "&radius=" + params.radius).toURL()
        response.setContentType("application/json")
        render url.getText()
    }

    def search = {
        def url = ("https://bie-ws.ala.org.au/ws/search.json?fq=${params.fq}&pageSize=${params.pageSize}&q=${params.q}").toURL()
        response.setContentType("application/json")
        render url.getText()
    }

    def species = {
        def url = ("https://bie-ws.ala.org.au/ws/species/${params.id}").toURL()
        response.setContentType("application/json")
        render url.getText()
    }

    def exploreTaxonGroup = {
        def url = ("https://biocache-ws.ala.org.au/ws/occurrence/search" + params.group + "?fq=geospatial_kosher%3Atrue&facets=species_group&lat=" + params.lat + "&lon=" + params.lon + "&radius=" + params.radius + "&start=" + params.start + "&pageSize=" + params.pageSize + "&common=" + params.common).toURL()
        response.setContentType("application/json")
        render url.getText()
    }

    def exploreGroup = {
        def url = ("https://biocache-ws.ala.org.au/ws/explore/group/" + params.group + "?fq=geospatial_kosher%3Atrue&facets=species_group&lat=" + params.lat + "&lon=" + params.lon + "&radius=" + params.radius + "&start=" + params.start + "&pageSize=" + params.pageSize + "&common=" + params.common).toURL()
        response.setContentType("application/json")
        render url.getText()
    }
    ...
    def exploreGroupWithGallery = {
        ...
        try {
            ...
            // do a HTTP POST to https://bie.ala.org.au/species/bulklookup with a JSON Array body
            def jsonOutput = new JsonOutput()
            def jsonBody = jsonOutput.toJson(guids)

            //do the http POST
            HttpClient http = new DefaultHttpClient()
            HttpPost post = new HttpPost("https://bie.ala.org.au/species/guids/bulklookup.json")
    ...
    def occurrenceSearch = {
        def url = ("https://biocache-ws.ala.org.au/ws/occurrences/search?q=" + params.q + "&fq=geospatial_kosher%3Atrue&facets=" + params.facets + "&lat=" + params.lat + "&lon=" + params.lon + "&radius=" + params.radius + "&start=" + params.start + "&pageSize=" + params.pageSize + '&fsort=index&flimit=300').toURL()
        response.setContentType("application/json")
        render url.getText()
    }
    ...
    def latestImages = {
        ...
        if (!start) start = 0
        def url = ("https://biocache-ws.ala.org.au/ws/occurrences/search?q=*:*&fq=multimedia:Image&facet=off&&sort=first_loaded_date&dir=desc&pageSize=" + pageSize + "&start=" + start).toURL()
        response.setContentType("application/json")
        render url.getText()
    }

    def exploreSubgroup = {
        def fullUrl = "https://biocache-ws.ala.org.au/ws/occurrences/search?q=*:*&lat=" + params.lat + "&lon=" + params.lon + "&radius=" + params.radius + "&pageSize=0&facets=species_guid&flimit=2000&fq=species_subgroup:" + URLEncoder.encode(params.subgroup, "UTF-8")
        try {
    ...
    def searchByMultiRanks = {
        def url = ("https://biocache-ws.ala.org.au/ws/occurrences/search?q=*:*&fq=geospatial_kosher%3Atrue&facets=species_subgroup&lat=" + params.lat + "&lon=" + params.lon + "&radius=" + params.radius + "&pageSize=0&flimit=1000").toURL()
        response.setContentType("application/json")
        def slurper = new JsonSlurper()
        def result = slurper.parseText(url.getText())
